...
- 每账号可设置模型的 USD/百万 Token 单价，仅估算后续有完整用量的请求；历史估算保留当时价格，未定价不显示为免费。费用不等同 Mirasim 的额度或 sub2 账单，详细口径见 USAGE.md。
- 新增账号搜索、状态筛选、到期/异常排序；概览汇总三天内到期、已到期、低额度和待检查状态，失效快照不会被当作实时额度告警。
- 批量检测支持仅检测筛选结果、停止后续任务、仅重试失败账号；显示逐账号进度，停止不会把已发出的请求误标为未发送。
- 模型目录新增文本/系列/启用状态/本次测试失败筛选及复制模型 ID，筛选使用已获取的数据，不新增模型请求。
- 新增匿名诊断 JSON 下载：只导出当前读取的状态与计数，排除账号名、profile、邮箱、地址、密钥、token 和原始日志；导出不发起检测。
- 自动刷新只查询当前页面，普通状态 15 秒同步；账号检测、窗口检查与升级进行中缩短到约 2 秒。模型目录约 30 秒更新；隐藏页面暂停轮询，回来后立即同步。
- 连接检测新增只读结果接口，保留最后结果供页面自动读取，不重新探测上游。窗口任务和模型测试只由用户操作触发，状态轮询不会重复发送推理。
- 模型测试结果直接显示在对应行；不清空表格，不替换未变化的模型行、账号选择项和表单，保存未完成的输入不被刷新覆盖。
- 丢弃已切换账号的旧请求结果，升级时账号状态读取失败仍能继续显示部署进度；查询失败保留现有数据并提示自动重试。
- 补齐 Linux 镜像、systemd 安装与源码包中的用量模块；升级保留账号和数据卷。用量从部署此版本后开始记录，不补旧版历史数据。

## 0.8.6 — 2026-09-26
- 新增各 Mira 账号会员状态：从 `/auth/me` 读取 `plan` / `plan_exp`，展示套餐、到期时间、已到期/未知/查询过期；不把 access token 的 exp 当会员期限。不暴露邮箱、原始用户 ID、token。
- 概览各账号行显示会员摘要；账号页支持单独刷新。额度备注同步时附带已知的会员到期时间，保持 sub2 计费字段不变。
- 新增每账号可选“自动开启额度窗口”：默认关闭，网页选择模型与 5h/7d 后启用；按上游真实重置/空闲证据发送一次 `hi`，经过此账号密钥的实际桥接器、Messages 协议、max_tokens=16，不直连绕过账号路由。
- 同时到期合并一次；每账号默认 6 次/滚动 24h，上限 8 次，最小间隔 5 分钟。发送前落盘、按身份绑定、原子锁、重启防重复、业务流量预检、额度阻塞和失败退避；模糊结果与窗口已确认分开记录。
- 强制中断遗留锁或持久化记录丢失时停止发送并显示原因。新添加 profile 不继承已启用的自动任务。
- 补齐 Docker 和 systemd 安装需要的新模块及网页文件。操作与验证边界见 MEMBERSHIP-WINDOWS.md。

## 0.8.5 — 2026-09-26
- 手动“强制检查更新”绕过宿主机 60 秒缓存；显示检查时间、缓存状态和发布源类型，检查失败不再保留旧版本为“最新”。
- GitHub latest 源先查询 Release API，再获取同一仓库具体 tag 的清单；带缓存重验证请求头和随机查询参数，核对 tag/清单版本及镜像摘要。API 不可达时明确标记下载入口回退，不静默使用旧缓存。
- 固定版本地址明确显示“不会自动发现新版”，管理员可切换为同一 GitHub/GHCR 仓库的 latest，不能从网页传入任意 URL。已有自定义源保持不变。
- 当前运行版本高于发布源时单独提示，避免标成最新版。
- 安装器增加 `--follow-latest`，便于旧工具无法发现新版时一次修复宿主机工具和同仓库发布源；不改账号、数据卷或镜像仓库。见 UPDATE-TROUBLESHOOTING.md。

## 0.8.4 — 2026-09-26
- 只读刷新不再占部署/账号修改互斥锁，修复自动刷新与 OAuth 回调提交冲突；升级、容器启停和账号配置修改仍串行保护。
- 当前账号增加“查看/复制 bridge 密钥”：仅管理权限按需读取，不出现在概览和审计记录中，60 秒后或切换账号/页面时清除显示。sub2 上游账号已经自动填入此密钥；它不是 sub2 客户端用户 Key。
- 当前账号及全部账号增加连接检测：带各自密钥经过真实 bridge `/v1/models`，另从 sub2 发起目录请求检查反向连接。显示各段结果、时间与模型数，不生成推理、不强制恢复暂停账号；同账号重复检测合并并缓存 10 秒。
- 回调提交响应丢失时查询同一个登录会话，区分等待回调、验证中、已保存和失败；已保存不会再要求重新授权，托管重试幂等。新增“查询登录状态”按钮。
- 清理托管注册成功后的等待定时器。

## 0.8.3 — 2026-09-26
- 修正 0.8.2 的登录声明：当时网页没有 GitHub 选项，后台也拒绝 GitHub。此版补齐两层支持；Google/GitHub 真授权入口已验证返回 307，最终账号授权仍需用户完成。
- Profile 可留空自动生成，邮箱形式自动规范化；移除会抢在提交处理之前拒绝这些输入的 HTML 校验。邮箱验证码暂在网页禁用。
- 修复部署就绪条件：账号暂停、未接入 sub2 或上游暂不可用不再触发代码回退；检查本地鉴权状态响应与运行版本，单独记录调度状态。没有响应、无效响应或版本不符仍拒绝部署。
- 回退失败时保留只读诊断；错误显示具体步骤/目标和固定错误码，不输出 Docker 原始内容或凭证。任务已接受与执行完成分开显示，运行版本未知不再标成最新版。
- 修复 ZIP 将 PNG 当 UTF-8 文本损坏的问题，CI 逐文件比对源字节；图标随宿主机安装/自更新恢复。
- 新增旧后台锁定的恢复入口 `install-panel.py --repair-recovery`，保留快照和凭证，重试恢复容器；见 RECOVERY.md。

## 0.8.2 — 2026-09-26
- 尝试修复 profile 输入与登录错误提示；本版仍漏接 GitHub，并受浏览器原生表单校验影响（在 0.8.3 完整修复）。
