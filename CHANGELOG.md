...
## 0.8.2 — 2026-09-26
- 修复网页新增 Google/GitHub 账号时邮箱被浏览器填入 profile 的问题；自动生成合法 profile，并保留明确的参数错误提示。
- 保留 0.8.1 的动态模型目录、额度同步和响应保真修复。

## 0.8.1 — 2026-09-26
- **模型目录和协议保真修正**：支持 `data/models/items/result` 目录形状，默认不隐藏 fable 或新模型；没有本地过滤时原样转发 `/v1/models`，只在显式策略命中时重写目录。网页和 sub2 同步使用同一目录解析器。
- **额度快照增强**：兼容百分比窗口、未发布预算、paid/status 字段；结构化剩余比例和更新时间继续写入 sub2 账号备注，且不修改 sub2 本地计费余额。
- **独立账号登录提示修正**：邮箱不再被浏览器填入 profile 目录名；页面自动生成合法 profile，并向用户显示具体的参数错误。
- **响应头最小化**：移除 bridge 自定义请求 ID 响应头，流式内容和模型响应字段仍按上游透传；内部错误 ID 只留在本地诊断状态。
