...
只替换 bridge 镜像，不初始化或删除数据卷，不覆盖账号配置、登录态或 sub2 分组。凭证继续由服务器保存和续期。网络升级不会同步宿主机目录里的源码、文档、Compose 模板或部署工具本身；未来需要更改这些文件的版本会另行注明。不要在网络升级后随手执行旧目录的 `docker compose build`，否则会用旧源码覆盖本地镜像。
