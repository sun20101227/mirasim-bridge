...
把 Mirasim 的 **Claude、GPT、DeepSeek、Kimi** 模型桥接到固定 HTTP 端口，通过标准上游账号接入原版 sub2api。当前版本 **0.8.5**，支持网页管理、网络升级接口、**多个 Mira 账号托管在同一个 bridge**（sub2api 按密钥区分）、额度备注同步、独立容器和 systemd，不需要更换 sub2api 或占用现有插件能力。

**0.8.5**：修复检查更新缓存与固定发布源提示。手动强制检查会重新查询，显示源类型和检查时间；仍显示旧版本的服务器请按 [更新检测修复步骤](UPDATE-TROUBLESHOOTING.md) 更新宿主机工具。
