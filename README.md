...
把 Mirasim 的 **Claude、GPT、DeepSeek、Kimi** 模型桥接到固定 HTTP 端口，通过标准上游账号接入原版 sub2api。当前版本 **0.8.4**，支持网页管理、网络升级接口、**多个 Mira 账号托管在同一个 bridge**（sub2api 按密钥区分）、额度备注同步、独立容器和 systemd，不需要更换 sub2api 或占用现有插件能力。

**0.8.4**：新增每账号密钥查看/复制、当前/全部账号连接检测；修复自动刷新与登录回调抢锁，回调响应丢失后自动查询保存结果。仍处于“回退未完成”的旧后台，请先按 [恢复步骤](RECOVERY.md) 操作；正常后台可直接网页升级。
