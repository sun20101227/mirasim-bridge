...
把 Mirasim 的 **Claude、GPT、DeepSeek、Kimi** 模型桥接到固定 HTTP 端口，通过标准上游账号接入原版 sub2api。当前版本 **0.8.7**，支持网页管理、网络升级接口、**多个 Mira 账号托管在同一个 bridge**（sub2api 按密钥区分）、额度备注同步、独立容器和 systemd，不需要更换 sub2api 或占用现有插件能力。

**0.8.7**：新增按账号、模型和时间查看 Token 消耗、每日趋势、请求明细和 CSV 导出；支持自定义单价估算，记录保留 30 天，未知用量明确标记。网页改为局部自动刷新，新增账号筛选、提醒、批量检测控制、模型查找和匿名诊断导出。操作与费用口径见 [用量统计](USAGE.md) · [网页管理](PANEL.md)。已有网页升级服务可在“版本与升级”检查并升级，成功后刷新网页。

**0.8.6**：新增每账号会员到期展示，以及可选的 5h/7d 自动窗口任务（按需发送一次 hi，默认关闭、限制次数、持久化防重、复查确认）。使用方法和计费说明见 [会员与窗口](MEMBERSHIP-WINDOWS.md)。
...
0.8.7：172 项 Node 回归、57 项 Python 测试、67 项离线自测，覆盖按账号用量隔离、累计 Token/缓存口径、断流缺失标记、并发记录与重启恢复、单价历史、CSV 公式防护和只读刷新。Chrome 手机/桌面检查覆盖局部刷新、账号切换、单价表单与 CSV 下载。用量转发测试使用本地 mock 上游验证原始回复字节；不能据此确认 Mirasim 官方扣费或补回旧版未记录的数据。
