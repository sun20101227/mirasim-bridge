...
class TerminalEvents {
  constructor(kind) { this.kind = kind; this.decoder = new StringDecoder('utf8'); this.buffer = ''; this.done = false; this.failed = false; this.recognized = false; }
  push(chunk) {
...
async function pipeEvents(upstream, downstream, kind, { begin = () => {}, firstEventTimeoutMs = 60000 } = {}) {
  const events = new TerminalEvents(kind);
  let pending = [], bytes = 0, started = false;
