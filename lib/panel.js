...
function createPanel(cfg, ctx, deps = {}) {
  ...
  const clean = () => {
    ...
    for (const [key, item] of connectionChecks) if (!item.pending && Date.now() - item.at >= 10000) connectionChecks.delete(key);
  };
  ...
  return {
    ...
    async call(operation, data = {}) {
      ...
      if (operation === 'window-keeper') {
        ...
        if (data.enabled === undefined) return windowKeeper.summarize(a.cfg, a.ctx.windowKeeperStatus);
      ...
      if (operation === 'account/check') {
        ...
        entry.promise = (async () => {
          ...
          return { account: a.key, checked_at: new Date().toISOString(), ok: forward.ok && reverse.ok !== false, bridge: forward, sub2api: reverse };
