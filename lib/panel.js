...
function createPanel(cfg, ctx, deps = {}) {
  ...
  return {
    ...
    async call(operation, data = {}) {
      ...
      if (operation === 'status') return { version: b.VERSION || null, quota: ctx.quota || { available: false, stale: true }, schedulable: ctx.sm?.desired || 'unmanaged', reachable: Boolean(ctx.reachable), inflight: ctx.inflight, uptime_sec: Math.round((Date.now() - ctx.startedAt) / 1000) };
      if (operation === 'summary') return {
        account_name: cfg.sub2api.account_name, group_ids: cfg.sub2api.group_ids,
        public_base_url: cfg.sub2api.public_base_url, max_concurrency: cfg.forward.max_concurrency,
      ...
      if (operation === 'model') {
        ...
        const next = b.deepMerge({}, cfg); next.constraints.disabled_models = [...disabled];
        b.validateConfig(next); atomicConfig(file, next);
        cfg.constraints.disabled_models = next.constraints.disabled_models;
      ...
      if (operation === 'login/start') {
        ...
        const profileCfg = profileConfig(cfg, data.profile, { 'account-name': data.account_name,
          'public-base-url': data.public_base_url, 'group-id': data.group_id, port: data.port });
...
async function handle(req, res, cfg, ctx) {
  ...
  try {
    let raw = ''; for await (const chunk of req) { raw += chunk; if (Buffer.byteLength(raw) > 160000) throw denied('请求过大'); }
